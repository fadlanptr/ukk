<template>
  <footer>
    <div class="footer bg-dark text-white footer-top py-3">
      <div class="container">
        <div class="row">
          <div class="col-md-6">
            <p><strong>SMKN 4 Tasikmalaya</strong></p>
            <p>Jl Depok Kel. Sukamenak Kec. Purbaratu<br>
              Kota Tasikmalaya 46196.
            </p>
            <p>+62265312059</p>
            <p>info@smkn4-tsm.sch.id</p>
          </div>
          <div class="col-md-6 text-md-end">
            <p><strong>Media Sosial</strong></p>
            <p><i class="fab fa-facebook"></i> <a class="text-white" href="https://www.facebook.com/4smkn?mibextid=ZbWKwL"> Facebook</a></p>
            <p><i class="fab fa-instagram"></i> <a class="text-white" href="https://www.instagram.com/smkn4tsmofficial?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=="> Instagram</a></p>
          </div>
        </div>
      </div>
    </div>
    <div class="footer-bottom bg-black text-white text-center py-2">
      <p class="mb-0">Copyright © 2024 SMKN 4 TASIKMALAYA</p>
    </div>
  </footer>

</template>

<style scoped>
/* .footer {
  background-color: #D9D9D9;
} */
</style>